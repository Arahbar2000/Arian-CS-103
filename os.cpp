...
OS::~OS(){
	...
	{
		if(dataStack.top() -> nodeData -> isFolder == true)
		{
			delete dataStack.top() -> nodeData -> childList;
		}
...
Node* OS::search_item(string fname){
	Node* top = wd.top() -> nodeData -> childList -> top();
    return wd.top() -> nodeData -> childList -> search(top, fname);
}
...
void OS::del(string fname){
	Node* top = wd.top() -> nodeData -> childList -> top();
    if(wd.top() -> nodeData -> childList -> search(top, fname) == NULL)
    {
    	cout << "Error: cannot find file or directory " << "'" << fname << "'" << endl;
    }
    else
    {
    	Node* target = wd.top() -> nodeData -> childList -> search(top, fname);
    	wd.top() -> nodeData -> childList -> deleteNode(target);
    }
}
...
void OS::create_item(string fname, bool isFolder){
    Node* top = wd.top() -> nodeData -> childList -> top();
    MList* list = wd.top() -> nodeData -> childList;
...
void OS::file(string fname, bool isRead){
	Node* top = wd.top() -> nodeData -> childList -> top();
    MList* list = wd.top() -> nodeData -> childList;
    if(list -> search(top, fname) == NULL)
    {
    	cout<<"Error: cannot find file '"<<fname<<"'"<<endl;
    }
    else if(list -> search(top, fname) -> nodeData -> isFolder == true)
    {
    	cout<<"Error: '"<<fname<<"' is not a file"<<endl;
    }
    else
    {
    	Node* target = list -> search(top, fname);
    	if(isRead == true)
    	{
    		cout << target -> nodeData -> text << endl;                                                                                                 
    	}
    	else
    	{
    		cout<<"$> ";	
    		string input;
    		getline(cin,input);
    		target -> nodeData -> text = input;
    		target -> nodeData -> size = input.length();
    	}
    }
}
...
void OS::cd_dir(string fname){
	if(fname == ".."){
		...
		*/
		...
		else if(list -> search(top, fname) -> nodeData -> isFolder == false)
		{
			cout<<"Error: '"<<fname<<"' is not a directory"<<endl;
		}
		else
